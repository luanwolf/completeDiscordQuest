...
O script valida Git, Node 22+ e `pnpm` global antes de fazer qualquer alteração. Depois, reusa ou clona o Vencord, atualiza este plugin em `src\userplugins` a partir do GitHub (mesmo com mudanças locais), roda `pnpm install --frozen-lockfile` e `pnpm build`, injeta no Discord, liga o plugin e reabre o cliente.
